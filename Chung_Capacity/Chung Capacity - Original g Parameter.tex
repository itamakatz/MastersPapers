...
.\\\par	In the optimal case where $l=l^{*}=\lStar$, we can calculate the following expressions for the $\lambda$'s
...
\\				& = & g(0) \cdot{\prod_{\tau=0}^{t} \left[\frac{(S_{0}-S_{1})^{2}(g(\tau)+1)}{g(\tau)-1} \right]}
	\end{IEEEeqnarray*}
...
:\\	\begin{IEEEeqnarray*}{r/C/l}
		\log{\left[\frac{g(t+\Delta)}{g(t)}\right]} & = & \log{\left[1 + \frac{g(t+\Delta)-{g(t)}}{g(t)}\right]}
\\													& \simeq & \frac{g(t+\Delta)-{g(t)}}{g(t)}
	\end{IEEEeqnarray*}
...
An alternative approach to describe the recursive control is by presenting the following parameter of the probabilities
:\\	\curlyBracketFull
    {\f \Longrightarrow}
    {
		\pi_{0}(t)=\piF{0}
...
With this new parameterization, we can now give an alternative continuation to the Mutual Information we found in equation (\ref{eq:before_q})
:\\	\begin{IEEEeqnarray*}{r/C/l}
...
At this point we can again develop the Entropy of the channel and substitute with our current parameter of interest $f(t)$
:\\	\begin{IEEEeqnarray*}{r/C/l}
...
Now going back to equation (\ref{eq:I_H_delta_change}), there we argued that the change in the certainty (or entropy) can be described as the decrease of information in each step of time, though in our case we are using the new parameter $f(t)$
:\\	\begin{IEEEeqnarray*}{r/C/l}
		H(f(t+\Delta))  & = & H(f(t))-\Delta I(f(t))
\\		                & \xrightarrow{} & 
	\end{IEEEeqnarray*}
\\ 
